  ...
  return (
    <div className="flex items-center gap-2">
      {comMes ? (
        <div className="flex items-center gap-1 rounded-xl border border-border bg-card p-1">
          ...
          <Select value={String(mes)} onValueChange={(v) => setMes(Number(v))}>
            <SelectTrigger className="h-8 w-[130px] border-0 bg-transparent shadow-none focus:ring-0">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {MESES.map((m, i) => (
      ...
      <Select value={String(ano)} onValueChange={(v) => setAno(Number(v))}>
        <SelectTrigger className="h-10 w-[96px]">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
          {anos.map((a) => (
      ...
      {itens.map((i) => (
        <div key={i.label}>
          <div className="mb-1 flex items-center justify-between text-sm">
            ...
            <span className="num font-medium">{brl(i.valor)}</span>
          </div>
          <div className="h-2 overflow-hidden rounded-full bg-secondary">
            <div
              className="h-full rounded-full transition-all"
              style={{ width: `${(i.valor / total) * 100}%`, backgroundColor: i.cor }}
            />
          </div>
        </div>
      ))}
